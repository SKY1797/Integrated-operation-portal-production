...
function updateHeader() {
    ...
    if (appState.module === 'HOME' || appState.module === 'LOGIN') {
        ...
        } else if (appState.module === 'OBSERVATION') { // Add this block
            const moduleAction = "updateState({module: 'OBSERVATION'})";
            moduleBtnHTML = `<button class="nav-pill-btn" onclick="${moduleAction}">${iconWalkdown} <span class="nav-text">Walkdown Register</span></button>`;
...
function renderHome() {
    return `
        ...
        <div class="item-grid" style="position: relative; z-index: 20; gap: 1.25rem">
            ...
            <button class="ui-card" style="flex-direction: column; align-items: normal;" onclick="updateState({module: 'OBSERVATION'})">
                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 0.75rem;">
                    <div class="card-icon-box" style="background: color-mix(in srgb, var(--module-electrical) 15%, var(--card)); color: var(--module-electrical); flex-shrink: 0; width: 2.5rem; height: 2.5rem;">${iconWalkdown}</div>
                    <div style="display: flex; flex-direction: column;">
                        <span class="card-title font-mono">Walkdown Register</span>
                        <span class="card-subtitle" style="margin-top:0.1rem;">Local area observations</span>
                    </div>
                </div>
    ...
    return `
        <div class="top-bar" style="display: flex; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
            ...
            <div style="background: var(--card); padding: 0.6rem 1rem; border-radius: 0.5rem; border: 1px solid color-mix(in srgb, var(--foreground) 10%, transparent); font-size: 0.85rem;">
                ...
                <span class="text-muted">Employee ID : </span> <strong class="font-mono text-foreground">${userId}</strong>
            </div>
        </div>
        ...
        <div class="ui-card" style="display: block; cursor: default; padding: 1.1rem;">
            <form id="obsForm" onsubmit="submitObservation(event)" style="display: flex; flex-direction: column; gap: 1rem;">
                ...
                <div>
                    ...
                    <input type="date" id="obsDate" class="ui-input" value="${today}" required style="width: 100% !important; max-width: 100% !important; box-sizing: border-box !important; margin: 0 !important; -webkit-appearance: none; appearance: none; padding: 0.75rem;">
                </div>
                ...
                <div>
                    <label class="list-label" style="display:block; margin-bottom:0.4rem;">Equipment Issue</label>
                    <textarea id="obsEquip" class="ui-input" rows="2" style="width: 100%; resize: vertical; padding: 0.75rem;" placeholder="Equipment defects, abnormal noise, vibration..."></textarea>
                </div>

                <div>
                    <label class="list-label" style="display:block; margin-bottom:0.4rem;">Steam / Water Leakages</label>
                    <textarea id="obsLeak" class="ui-input" rows="2" style="width: 100%; resize: vertical; padding: 0.75rem;" placeholder="Passing valves, flange leakages, gland leaks..."></textarea>
                </div>

                <div>
                    <label class="list-label" style="display:block; margin-bottom:0.4rem;">Lighting Issue</label>
                    <textarea id="obsLight" class="ui-input" rows="2" style="width: 100%; resize: vertical; padding: 0.75rem;" placeholder="Fused bulbs, dark areas, panel indications..."></textarea>
                </div>

                <div>
                    <label class="list-label" style="display:block; margin-bottom:0.4rem;">Housekeeping Requirement</label>
                    <textarea id="obsHouse" class="ui-input" rows="2" style="width: 100%; resize: vertical; padding: 0.75rem;" placeholder="Spillage, scrap, cleaning requirements..."></textarea>
                </div>

                <div>
                    <label class="list-label" style="display:block; margin-bottom:0.4rem;">Unsafe Condition</label>
                    <textarea id="obsUnsafe" class="ui-input" rows="2" style="width: 100%; resize: vertical; padding: 0.75rem;" placeholder="Missing gratings, open covers, safety hazards..."></textarea>
                </div>

                <button type="submit" id="obsSubmitBtn" class="submit-action-btn">
                    Submit Report
                </button>
...
window.submitObservation = async function (e) {
    ...
    const payload = {
        ...
        housekeeping: document.getElementById('obsHouse').value,
        unsafeCondition: document.getElementById('obsUnsafe').value
    };
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // Optional: reset date to today after form reset
